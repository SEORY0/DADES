...
- 같은 base끼리 묶고, `id === base`인 모델을 대표로 삼는다. base ID가 카탈로그에 없는 묶음(별칭만 있거나 free 변형만 있는 경우)은 행으로 만들지 않는다. 사용량 패널은 경로 단위이므로 그대로 보인다.
...
- Terminal-Bench 4.0 패널: 모델별 최고 행 하나씩 상위 6개(같은 모델의 추론 강도별 행이 상위를 독차지하지 않게). 한 줄에 공식 순위 · 모델명 · 정확도, 아래 줄 작은 글씨로 에이전트명 · 추론 강도. 막대는 사용량 패널과 같은 트랙 스타일. 하단에 리더보드 원문 링크와 `updated_at` 날짜.
- 주간 사용량: 지금 그대로.
